...
def process_rdd(time, rdd):
  ...
  try:
    ...
    row_rdd = rdd.map(lambda w: Row(category=w[0], count=w[1]))
...
def filter_harsh_word(tweet, data):
  for word in data:
    if word[0] in tweet:
      return "!FILTEREDBAD " + tweet
  return "!FILTEREDNOTBAD " + tweet
...
words = bad_filtered.flatMap(lambda line: line.split(" "))
